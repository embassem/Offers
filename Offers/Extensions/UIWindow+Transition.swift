...
public extension UIWindow {
    ...
    func setRootViewControllerWithTransition(
        ...
        if let background = options.background {
            ...
            case .customView(let view):
                transitionWnd?.rootViewController =
                    UIViewController.newController(withView: view, frame: transitionWnd!.bounds)
        ...
        if let wnd = transitionWnd {
            DispatchQueue.main.asyncAfter(deadline: (.now() + 4.0 + options.duration), execute: {
